                    ...
                    <?php if ($priorityMode === 'disabled'): ?>
                            <form method="POST" action="<?= url('/scheduling/pass-role') ?>" class="m-0">
                                ...
                                <button type="submit" class="btn btn-outline-dark btn-sm"
                                    onclick="return confirm('هل تريد تمرير الدور للعضو التالي؟')">
...
<?php if ($isAdmin && empty($myCourses)): ?>
        ...
        <div class="callout callout-info" style="border-right-color: #2196F3;">
            ...
            <p class="mb-0 text-muted" style="font-size: .9rem;">يمكنك تسكين المحاضرات نيابة عن الأعضاء من صفحة <a
                    href="<?= url('/admin-scheduling') ?>">التسكين الإداري</a> أو من <a
                    href="<?= url('/grid-scheduling') ?>">التسكين الرسومي</a>.</p>
        </div>
<?php endif; ?>
...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-filter ml-1"></i> تصفية</h3>
    </div>
            ...
            <div class="row">
                ...
                <div class="col-md-4 d-flex align-items-end">
                    <div class="form-group mb-0">
                        ...
                        <a href="<?= url('/scheduling') ?>" class="btn btn-secondary" id="btnResetFilter">إعادة
                            تعيين</a>
                    </div>
                </div>
            </div>
...
<?php if ($canSchedule): ?>
        <div class="card card-primary card-outline" id="addEntryCard" <?= empty($myCourses) ? 'style="display:none"' : '' ?>>
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-plus ml-1"></i> إضافة محاضرة جديدة</h3>
            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="classroom_id" class="form-control select2" required>
                                    ...
                                    <?php foreach ($classrooms as $cr): ?>
                                            <option value="<?= $cr['classroom_id'] ?>"><?= e($cr['classroom_name']) ?>
                                                (<?= e($cr['classroom_type'] ?? '') ?>)</option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="session_id" class="form-control select2" required>
                                    ...
                                    <?php foreach ($sessions as $sess): ?>
                                            <option value="<?= $sess['session_id'] ?>"><?= e($sess['day']) ?> —
                                                <?= e($sess['session_name'] ?? '') ?>
                                                (<?= $sess['start_time'] ?>-<?= $sess['end_time'] ?>)</option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
...
<div id="schedulingResults">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title"><i class="fas fa-list ml-1"></i>
                <?= $isAdmin && empty($myCourses) ? 'نظرة عامة على الجدول' : 'محاضراتي في الجدول' ?> (<span
                    id="entriesCount"><?= count($myEntries) ?></span>)</h3>
        </div>
                        ...
                        <tbody id="entriesBody">
                            <?php foreach ($myEntries as $i => $e): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($canSchedule): ?>
                                                    <a href="<?= url("/scheduling/{$e['timetable_id']}/edit") ?>"
                                                        class="btn btn-warning btn-xs"><i class="fas fa-edit"></i></a>
                                                    <form method="POST" action="<?= url("/scheduling/{$e['timetable_id']}/delete") ?>"
                                                        class="d-inline">
                                                        <?= csrf_field() ?>
                                                        <button type="submit" class="btn btn-danger btn-xs btn-delete"><i
            ...
            <?php else: ?>
                    <?php if ($isAdmin && empty($myCourses)): ?>
                            <div class="text-center p-5">
                                ...
                                <a href="<?= url('/admin-scheduling') ?>" class="btn btn-primary btn-sm"
                                    style="text-decoration: none;"><i class="fas fa-user-shield ml-1"></i> التسكين الإداري</a>
                                <a href="<?= url('/timetable') ?>" class="btn btn-outline-secondary btn-sm mr-2"
                                    style="text-decoration: none;"><i class="far fa-clock ml-1"></i> عرض الجدول</a>
                            </div>
                    <?php else: ?>
