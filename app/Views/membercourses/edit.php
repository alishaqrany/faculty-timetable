...
$selectedAssignmentType = old('assignment_type', $currentType);
$isPractical = $selectedAssignmentType === 'عملي';
$selectedSubjectId = (int)old('subject_id', $course['subject_id']);
// Determine practical scope: if practical and has division_id (no section_id) → division-level
$practicalScope = 'section';
if ($isPractical && !empty($course['division_id']) && empty($course['section_id'])) {
    $practicalScope = 'division';
}
$selectedPracticalScope = old('practical_scope', $practicalScope);
$isPracticalSection = $isPractical && $selectedPracticalScope === 'section';
$isPracticalDivision = $isPractical && $selectedPracticalScope === 'division';
$selectedSectionId = (int)old('section_id', (int)($course['section_id'] ?? 0));
$selectedPracticalDivisionId = (int)old('practical_division_id', (int)($course['division_id'] ?? 0));
?>
    ...
    <div class="card card-warning">
        ...
        <form method="POST" action="<?= url("/member-courses/{$course['member_course_id']}") ?>">
            ...
            <div class="card-body">
                    ...
                    <select name="subject_id" id="subjectSelect" class="form-control select2" required>
                        ...
                        <?php foreach ($subjects as $s): ?>
                            <option value="<?= $s['subject_id'] ?>"
                                    ...
                                    <?= $selectedSubjectId === (int)$s['subject_id'] ? 'selected' : '' ?>>
                ...
                <div class="form-group" id="practicalScopeGroup" style="<?= !$isPractical ? 'display:none;' : '' ?>">
                ...
                <div class="form-group" id="sectionGroup" style="<?= $isPracticalSection ? '' : 'display:none;' ?>">
                    <label>السكشن (مجموعة العملي) <span class="text-danger">*</span></label>
                    <select name="section_id" id="sectionSelect" class="form-control select2" <?= $isPracticalSection ? '' : 'disabled' ?>>
                        <option value="">-- اختر سكشن --</option>
                        <?php foreach ($sections ?? [] as $sec): ?>
                            <option value="<?= $sec['section_id'] ?>" <?= ($isPracticalSection && $selectedSectionId === (int)$sec['section_id']) ? 'selected' : '' ?>>
                                <?= e($sec['section_name']) ?> 
                ...
                <div class="form-group" id="practicalDivisionGroup" style="<?= $isPracticalDivision ? '' : 'display:none;' ?>">
                    <label>الشعبة (للعملي) <span class="text-danger">*</span></label>
                    <select name="practical_division_id" id="practicalDivisionSelect" class="form-control select2" <?= $isPracticalDivision ? '' : 'disabled' ?>>
                        <option value="">-- اختر شعبة --</option>
                        <?php foreach ($divisions ?? [] as $div): ?>
                            <option value="<?= $div['division_id'] ?>" <?= ($isPracticalDivision && $selectedPracticalDivisionId === (int)$div['division_id']) ? 'selected' : '' ?>>
                                <?= e($div['division_name']) ?> — <?= e($div['department_name']) ?> / <?= e($div['level_name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
